...
export class BlockRevisionAclAdapter {
  ...
  public async createBlockRevision(auth: AuthUser, blockId: string, body: CreateBlockRevisionDto) {
    const permission = auth.create(null, AccessResource.BLOCK_REVISION);
    if (!permission.granted) throw new ForbiddenException();

    const result = await this.blockRevision.create(blockId, body, auth.id);
